...
public class GroupChatManager extends QBMessageListenerImpl<QBGroupChat> implements ChatManager {
    ...
    public GroupChatManager(Context chatActivity) {
    ...
    private void join(final QBGroupChat groupChat, final QBEntityCallback callback) {
        ...
        groupChat.join(history, new QBEntityCallbackImpl() {
            ...
            public void onSuccess() {
                ...
                groupChat.addMessageListener(GroupChatManager.this);

                ((Activity) chatActivity).runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        callback.onSuccess();
                    }
    ...
    public void processMessage(QBGroupChat groupChat, QBChatMessage chatMessage) {
        ...
        //((Activity) chatActivity).showMessage(chatMessage);
    }
