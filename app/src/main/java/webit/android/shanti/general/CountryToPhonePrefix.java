...
public class CountryToPhonePrefix {
    ...
    public static int getKeyByPrefix(String prefix) {
        int key = -2;
        int i = 0;
        Map<String, String> ezerMap = new HashMap<>();
        for (String value:map.keySet()) {//עובר על כל ה key
            i++;
            if (map.get(value).equals(prefix))//בודק עם ה value של ה key שווה למה שקיבלנו
                key = i;
        }
        return key;
    }
